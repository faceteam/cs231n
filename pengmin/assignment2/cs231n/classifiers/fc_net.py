...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    hidden = {}
    num_of_layers = self.L
    hidden['h0'] = X.reshape(X.shape[0], np.prod(X.shape[1:]))
    # Dropout for input layer
    if self.use_dropout:
      hdrop, cache_hdrop = dropout_forward(hidden['h0'], dropout_param)
      hidden['hdrop0'] = hdrop
    ...
    for i in range(num_of_layers):
      ...
      # For other layers
        ...
        # 这里一定要注意dropout的顺序，在batchnorm之后
        if self.use_batchnorm:
        ...
        if self.use_dropout:
          hdrop, cache_hdrop = dropout_forward(h, dropout_param)
          hidden['hdrop'+str(idx)] = hdrop
          hidden['cache_hdrop'+str(idx)] = cache_hdrop

    scores = hidden['h'+str(num_of_layers)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    # Backward pass
    hidden['dh'+str(num_of_layers)] = dscores
    ...
    # Add regulariation on dW = dW + reg * W
