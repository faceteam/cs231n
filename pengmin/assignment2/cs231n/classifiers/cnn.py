...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    '''
    Pooling layer
    Input size: (N, F, H_, W_)
    Output size: (N, F, Hp, Wp)
    '''
    stride_pool = 2
    Hp = (H_ - 2) / stride_pool + 1
    Wp = (W_ - 2) / stride_pool + 1
  ...
  def loss(self, X, y=None):
    ...
    N, F, Hp, Wp = h1.shape
    h1 = h1.reshape((N, F*Hp*Wp)) ## Must reshape!!!
    h2, cache_h2 = affine_relu_forward(h1, W2, b2)
    h3, cache_h3 = affine_forward(h2, W3, b3)
    scores = h3
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss += reg_loss
    dh3, dW3, db3 = affine_backward(dscores, cache_h3)
    dW3 += self.reg * W3
    dh2, dW2, db2 = affine_relu_backward(dh3, cache_h2)
    dW2 += self.reg * W2

    dh2 = dh2.reshape(N,F,Hp,Wp) ## Must reshape back!!!
    dX, dW1, db1 = conv_relu_pool_backward(dh2, cache_h1)
    dW1 += self.reg * W1
