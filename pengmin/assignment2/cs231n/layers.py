...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (mu, xmu, carre, var, sqrtvar, invar, va2, va3, gamma, beta, x, bn_param)
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  # Backware pass
  mu, xmu, carre, var, sqrtvar, invar, va2, va3, gamma, beta, x, bn_param = cache
  eps = bn_param.get('eps', 1e-5)
  N, D = dout.shape
  # Step 9
  dva3 = dout
  ...
  # Step 7
  dxmu = invar * dva2
  dinvar = np.sum(xmu * dva3, axis=0)

  # Step 6
  dsqrtvar = -1.0 / sqrtvar**2 * dinvar
...
def conv_backward_naive(dout, cache):
  ...
  print 'hhh' ## There is something wrong!!!
  dx = np.zeros((N, C, H, W))
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    mu = (1. / D) * np.sum(x, axis=(0,2,3)).reshape(1,C,1,1)
    var = (1. / D) * np.sum((x-mu)**2, axis=(0,2,3)).reshape(1,C,1,1)
