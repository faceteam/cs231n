...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_scores=exp_scores/row_sum
    loss=-np.sum(np.log(softmax_scores[range(N),list(y)]))
    loss=loss/N
    ...
    dscores[range(N), list(y)] += -1      #反向传播
    grads['W2'] = 1.0/N * h1.T.dot(dscores) + reg * W2
    grads['b2'] = 1.0/N * np.sum(dscores, axis=0)

    dh1 = dscores.dot(W2.T)
    dadd1 = (X.dot(W1)+b1 > 0)*dh1
    grads['W1'] = 1.0/N * X.T.dot(dadd1) + reg * W1
    grads['b1'] = 1.0/N * np.sum(dadd1, axis=0)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def predict(self, X):
    ...
    ###########################################################################
    h11 = np.maximum(0,(np.dot(X, self.params['W1']) + self.params['b1']))
    scores = np.dot(h11, self.params['W2']) + self.params['b2']
